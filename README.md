...
| `immich_install` | Install the [Immich](https://immich.app/) photo and video manager via Docker against an external PostgreSQL, such as `postgres_pgbackrest_install` built on Immich's own postgres image. |
| `immich_restic_backup` | Automated Immich media backups to an S3 compatible backend with [Restic](https://restic.net/), including scheduled `restic check` verification and bootstrap restore. The database is covered by `postgres_pgbackrest_install`. |
